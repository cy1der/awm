...
require('modules.screenful.screens_db')

local card = 'card0'
local dev = '/sys/class/drm/'
local configPath = awful.util.getdir("config") ..
                       "modules/screenful/screens_db.lua"
...
local function connectedOutputs(path, card)
    ...
    while true do
        ...
        if isOutputConnected(output) then result[output] = true end
    end

    return result
end

local function emptyStr(str) return str == nil or str == '' end
...
local function getScreenId(output)
    ...
    if isOutputConnected(output) then
        ...
        for i = 12, 17 do
            code = id:byte(i)
            if code then screenId = screenId .. code end
        end
    end
    return screenId
end
...
local function getXrandrOutput(outputPath, outCard)
    ...
    if outputMapping[drmName] then return outputMapping[drmName] end
...
local function setupScreen(xrandrParams) os.execute('xrandr ' .. xrandrParams) end
...
local function performConfiguredAction(screenId, action, xrandrOut)
    ...
    if screenId then
        ...
        if configuration then
            ...
            if tostring(screenId):len() ~= 0 and
                not hasConfigurationFor(screenId) then
                naughty.notify({
                    text = 'Append new configuration for screen id: ' ..
                        screenId
                })
    ...
    if xrandrOpts then setupScreen(xrandrOpts) end
end
...
function updateScreens(changedCard)
    ...
    for out in pairs(mergedOutputs) do
        if not outputs[out] then
            enableOutput(out, changedCard)
        elseif not newOutputs[out] then
